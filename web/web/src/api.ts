...
// ---------------------------------------------------------------------------
// Manual override endpoints — close position(s), manual open (override AI)
// ---------------------------------------------------------------------------
...
async function postJson<T>(url: string, body: unknown): Promise<T> {
  ...
  if (!res.ok) {
    throw new Error((data as { reason?: string }).reason || `${res.status}: ${res.statusText}`);
  }
  return data as T;
}

export async function closePosition(id: number): Promise<ActionResult> {
  const res = await fetch(`${BASE}/trade/positions/${id}`, { method: "DELETE" });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
...
export async function closeAllPositions(positions: Position[]): Promise<{ closed: number; failed: number }> {
  let closed = 0;
  let failed = 0;
  await Promise.all(
...
export async function placeManualOrder(payload: ManualOrderPayload): Promise<ActionResult> {
  return postJson<ActionResult>(`${BASE}/trade/manual`, { ...payload, confidence: 1.0 });
}

// ---------------------------------------------------------------------------
// Runtime trading control — mode / leverage / paused / risk params
// ---------------------------------------------------------------------------

export type TradingMode = "ai" | "trend" | "breakout" | "rl" | "hybrid";
...
export async function getTradingControl(): Promise<TradingControl | null> {
  try {
    const data = await fetchJson<TradingControl>(`${BASE}/trade/control`);
    return tagReal(data);
  } catch {
    return null;
  }
...
export async function setTradingControl(payload: TradingControlPayload): Promise<{
  success: boolean;
  changes: string[];
  note: string;
}> {
  return postJson(`${BASE}/trade/control`, payload);
}
...
export async function getPositionDetail(id: number): Promise<PositionDetail> {
  const data = await fetchJson<PositionDetail>(`${BASE}/trade/positions/${id}`);
  return tagReal(data);
}

// ---------------------------------------------------------------------------
// Decision history, AI stats, market ticker — info panels
// ---------------------------------------------------------------------------
...
export async function getDecisionHistory(limit: number = 50): Promise<AiDecisionRecord[]> {
  try {
    const data = await fetchJson<{ decisions: AiDecisionRecord[]; count: number }>(
      `${BASE}/trade/decisions?limit=${limit}`
    );
    return tagRealArray(data.decisions ?? []);
  } catch {
    return [];
  }
}
...
export interface AiStats {
  ...
  current_position_scalar: number;
}

export async function getAiStats(): Promise<AiStats | null> {
  try {
    const data = await fetchJson<AiStats>(`${BASE}/ai/stats`);
    return tagReal(data);
  } catch {
    return null;
  }
...
export async function getMarketTicker(symbol: string = "BTC/USDT:USDT"): Promise<MarketTicker | null> {
  try {
    const data = await fetchJson<MarketTicker>(`${BASE}/market/ticker?symbol=${encodeURIComponent(symbol)}`);
    return tagReal(data);
  } catch {
    return null;
  }
