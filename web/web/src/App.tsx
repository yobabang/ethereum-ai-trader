...
import { TradingControlPanel } from "./components/TradingControl";
...
export default function App() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [acct, pos, ord, eq, ai, ctrl] = await Promise.all([
        getAccount(),
        getPositions(),
        getOrders(50),
        getEquityHistory(7),
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <header className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          {control && (control.mode || control.leverage != null) && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-purple-900/50 text-purple-300 border border-purple-700">
              {control.mode ?? "—"} · {control.leverage ?? "—"}x
              {control.paused && <span className="text-yellow-300">· 已暂停</span>}
            </span>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
        ...
        <div className="lg:col-span-2">
          <KlineChart positions={positions} />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
          <ControlBar positions={positions} onAction={refresh} />
        </div>
      </div>

      {/* Info panels: AI decision history + AI stats + market ticker */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
      ...
      <PositionDetail position={detailPos} onClose={() => setDetailPos(null)} />
    </div>
  );
}
